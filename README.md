...
**N** : 要素(index)の数
  ...
  *W*/*A* : **O(N)** <br>
  *B* : **O(1)** <br>
  note : 線形探索。本実装ではデータ構造の都合上、二分探索などによる最適化はできない。 <br>
  
 - 範囲質問 (の解の導出)
   - 最初の処理 (新しく定義された検索範囲内) <br>
    *W*/*A* : **O(N)** <br>
    *B* : **O(1)** <br>
    note : 線形探索。データ構造の都合上、二分探索などによる最適化はできない。 <br>
...
#### 今後実装が期待できるもの
 ...
 - 線形探索処理の削除(二分探索やハッシュ探索による代替の考察)
 - 複数の抽出範囲の定義のサポート
 

### 今後すべきこと

 - 第一に、他の既存手法についてさらに調べ、随時実装やパフォーマンス比較を行いたい
 - 上記の"今後実装が期待できるもの"の実現に向けて、当コードの加筆及び書き直し
 - パフォーマンス比較のテストパターンの追加
