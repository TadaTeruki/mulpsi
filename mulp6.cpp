...
namespace spacial{
    ...
    /**/private:
        ...
        struct m_index{
            INDEX index;
            VALUE value;
            friend bool operator< (m_index a, m_index b) { return a.value < b.value; }
        };
        ...
        struct m_flag{
            ...
            bool extractable(){
                for(m_access ac; ac.enable(); ac.inc()){
                    if(flag[ac.f][ac.b] == false) return false;
                }
                return true;
            }
        };
        ...
        void update_extrange(std::initializer_list<VALUE> & _values){
            ...
            for(m_access ac; ac.enable(); ac.inc()) { extrange[ac.f][ac.b] = *ins_iter; ins_iter++; }
        }

        void init(){
            for(m_access ac; ac.enable(); ac.inc()) data[ac.f][ac.b].clear();
            flag_data.clear();
            m_container::clear();
        }

        void slide_insertion(m_access _ac){
            flag_data[(*iter[_ac.f][_ac.b]).index].flag[_ac.f][_ac.b] = true;
            if(flag_data[(*iter[_ac.f][_ac.b]).index].extractable() == true) m_container::insert((*iter[_ac.f][_ac.b]).index);
        }

        void slide_deletion(m_access _ac){
            if(flag_data[(*iter[_ac.f][_ac.b]).index].extractable() == true) m_container::erase((*iter[_ac.f][_ac.b]).index);
            ...
            for(m_access ac; ac.enable(); ac.inc()){
                ...
                if( iter[ac.f][ac.b] != data[ac.f][ac.b].begin()){
                    ...
                    while((*iter[ac.f][ac.b]).value > extrange[1-ac.f][ac.b] ) {
                        if(ac.f == 0) slide_deletion(ac);
                        else          slide_insertion(ac);
                        if(iter[ac.f][ac.b] != data[ac.f][ac.b].begin()) iter[ac.f][ac.b]--;
                        else break;
                    }
                }
                ...
                if( iter[ac.f][ac.b] != data[ac.f][ac.b].end()-- ){
                    while((*iter[ac.f][ac.b]).value <= extrange[1-ac.f][ac.b] ) {
                        if(ac.f == 1) slide_deletion(ac);
                        else          slide_insertion(ac);
                        if( iter[ac.f][ac.b] != data[ac.f][ac.b].end()-- ) iter[ac.f][ac.b]++;
                        else break;
                    }
                }
            }
    ...
    /**/public:
        ...
        mulp6(){
            for(m_access ac; ac.enable(); ac.inc()) extrange[ac.f][ac.b] = 0;
            init();
        }
        ...
        void insert(INDEX _index, std::initializer_list<VALUE> _values ){
            ...
            for(m_access ac; ac.enable(); ac.inc()){
                ...
                else iflag.flag[ac.f][ac.b] = false;
                
                ins_iter++;
            }

            flag_data.emplace( _index, iflag );
            if(iflag.extractable() == true) m_container::insert(_index);
            if(register_iter == true){ for(m_access ac; ac.enable(); ac.inc()) iter[ac.f][ac.b] = data[ac.f][ac.b].begin(); }
            slide_pointer();
        }
        ...
        void erase(INDEX _index){
            ...
            for(m_access ac; ac.enable(); ac.inc()){
                ...
                for(auto it = data[ac.f][ac.b].begin(); it != data[ac.f][ac.b].end(); it++){
                    if((*it).index == _index){
